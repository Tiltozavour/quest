...
> Для этого используетя символ "?" - val example:String? = null - строка null и это не вызовет ошбибку компилятора
...
> Удобный класс, в основном использую для хранения данных. В таком классе переопределены методы - ToString(), Equals(), HashCode(), Component(). Почему это удобно? Упрощние сравнение объектов и их сериализация, автоматически сгненнерированы сеттеры и геттеры для каждого поля
...
> Усовершенствованные enun, класс в который можно положить и другие class и объекты, ограниченное наследование, 
