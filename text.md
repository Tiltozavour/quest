...
>  Val известен во время runtime, const val - во время компиляциии\
...
<details>
  <summary> Что такое лямбда - выражения </summary>
...
<details>
  <summary> Что такое inline </summary>

Говорит функции вставить код лямбды прямо в месте вызова, тем самым уменьшая расходы на производительность

 </details>

 <details>
  <summary> Что такое noinline </summary>

Говорит конкректной лямбде не инлайнится 

**Зачем:**
- Если лямбда передаётся в другую не-inline функцию.
- Если нужно сохранить лямбду как объект (например, для отложенного выполнения).

```
inline fun process(noinline validator: () -> Boolean, action: () -> Unit) {
    if (validator()) action()
}
//validator не будет inline
```
 ...
 <details>
  <summary> Crossinline </summary>

> Запрещает return (локальный) внутри лямбды (чтобы избежать неожиданного выхода из внешней функции).

> Используется для параметров inline-функций, если лямбда передаётся в другой контекст (например, в другой inline-функцию или сохраняется в переменную).

**Зачем?**

Это «запрет на побег» (return) для лямбды, если её передают куда-то ещё.\
Нужен, чтобы код не сломался, когда лямбда выполняется не сразу.\

```
kotlin
inline fun runAsync(crossinline action: () -> Unit) {
    Thread { action() }.start()  // лямбда выполняется в другом потоке
}

fun main() {
    runAsync { 
        println("Done")
        // return  // Ошибка: нельзя использовать non-local return!
    }
}
```
...
inline fun <reified T> checkType(value: Any) {
    if (value is T) println("Correct type!")  // Проверка типа в runtime
}

fun main() {
    checkType<String>("Text")  // Выведет: Correct type!
    checkType<Int>("Text")     // Ничего не выведет
}
```
...
###  **Какие виды flow существуют** 
 ...
 <details>
  <summary> Холодные флоу (Flow) </summary>

- Начинает работать только если есть подписка (через терминальные операторв - типа collect)
- Закончат работу, как только отдадут все данные
- Каждая подписка, создает новый поток

  </details>

 <details>
  <summary> Горячие флоу (State и Shared ) </summary>

- Начинает работать независимо от подписчиков
- Никога не закончатся, продолжают работу независиммо есть данные или нет

  </details>

### Чем state отличается от shared flow

- У state всегда есть значение по умолчанию
- Параметр replay (значение количество последних значений) равен 1 и изменить нельзя - у shared 0, и можно менять
- Backpressure нельязя настройить, у shared - настраивается
- У state при emit_е одинаковых значений отработает один раз, у shared можно настроить дублирование (по умолчанию дублирует)
- У state есть свойство value
...
<details>
  <summary><h2> 📡 BroadcastReceiver </h2></summary>
...
**BroadcastReceiver — "слушатель" событий (системных или от приложений)**

> Реагирует на широковещательные сообщения (broadcasts) от системы или других приложений.

**Примеры событий:** входящий звонок, низкий заряд батареи, изменение режима "в самолёте", загрузка завершена.
...
<details>
  <summary><h2> 🗄️ ContentProvider </h2></summary>
...
**ContentProvider — "источник данных", который можно запрашивать извне**

> Предоставляет структурированный доступ к данным (например, к БД, файлам) для других приложений.

**Пример:** контакты, медиафайлы, пользовательские данные приложения.
...
- **Created (Создание):**
> View создается программно или загружается из XML-макета.

- **Attached (Присоединение):**
>  View присоединяется к родительскому контейнеру (например, к Layout)

- **Measured (Измерение):**
>  Система измеряет размеры и расположение View в контейнере, чтобы правильно разместить его на экране.

- **Layout (Размещение):**
> View размещается внутри родительского контейнера с учетом измерений, определенных на предыдущем этапе.

- **Draw (Отрисовка):**
 > View отрисовывается на экране.

- **Detached (Отсоединение):**
> View отсоединяется от родительского контейнера.

-  **Destroyed (Уничтожение):**
> View уничтожается, освобождая память и ресурсы, занимаемые этим элементом. 
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
<details>
  <summary><h2> 🔄 ЖЦ (DEPRECATED) </h2></summary>
...
<details>
  ...
  > Интерфейс - контракт методов - пример: репозиторий

  > Абстрактный класс - заготовка для основного класса - пример: базовый фрагмент
...
>  (Модель (бизнес-логика) - View (UI) - Коннтроллер(обрабатывает пользовательский ввод, управляет Model и обновляет View.)

**Проблемы**:если логика накапливается в Controller, его сложно поддерживать.\
...
**Пробелемы:** Presenter много знает о view, а view о Presenter\
**Как работает:**
```
...
Model - обычно в дата класс состояния
```
...
View - рендерит стейт на основе состояния (как мы обычно обрабатываем в активити)
Intent - события состояний (обычно сеалед классы)

```

sealed class UserIntent {
    object LoadUser : UserIntent()
    data class Retry(val userId: String) : UserIntent()
}
...
Как реализовано Synhronized в Coroutines (через монитор)
